...
import { baseUrlVendas, getParametersVendasPadrao } from '../util/constantes';
...
const PesquisaBarra = ({ pesquisaNome, pesquisaData, atualizaLista }) => {
  ...
  useEffect(() => {
    ...
    axios.get('https://back.pedrohenriq1921.repl.co/api/clientes', config)
      .then((response) => {
...
const TableVendas = ({data, setData, atualizaLista}) => {
  ...
  useEffect(() => {
    ...
    axios.get('https://back.pedrohenriq1921.repl.co/api/produtos', config)
      .then((response) => {
        if (response.status === 200) {
  ...
  const obterProdutos = (vendaId) => {
    return new Promise((resolve, reject) => {
      axios
        .get(`https://back.pedrohenriq1921.repl.co/api/item-vendas?filters[venda][id][$eq]=${vendaId}&populate=*`, config)
        .then((response) => {
          if (response.status === 200) {
            const dadosProdutos = response.data.data;
  ...
  const editarVenda = (vendaEditada) => {
    ...
    axios.put(`https://back.pedrohenriq1921.repl.co/api/vendas/${vendaEditada.key}`, { data: camposEditados }, config)
      .then((response) => {
        if (response.status === 200) {
  ...
  const editarItemVenda = (itemEditado) => {
    ...
    axios.put(`https://back.pedrohenriq1921.repl.co/api/item-vendas/${itemEditado.key}`, { data: camposEditados }, config)
      .then((response) => {
        if (response.status === 200) {
  ...
  const excluirVenda = (vendaId) => {
    axios.delete(`https://back.pedrohenriq1921.repl.co/api/vendas/${vendaId}`, config)
      .then((response) => {
        atualizaLista();
      })
      .catch((error) => {
  ...
  const excluirProduto = (produtoId) => {
    axios.delete(`https://back.pedrohenriq1921.repl.co/api/item-vendas/${produtoId}`, config)
    .then((response) => {
        ...
        <div>
          ...
          {produtos.map((produto, index) => (
            <div key={index}>
              ...
              <strong>Custo de Venda (R$):</strong> {produto.custo_venda}<br />
              <strong>Preço de Venda (R$):</strong> {produto.preco_venda}<br />
...
const Teste = () => {
  ...
  const pesquisaNome = (clienteId) => {
    axios.get(`https://back.pedrohenriq1921.repl.co/api/vendas?sort=data:desc&filters[cliente][id][$eq]=${clienteId}&populate=*`, config)
      .then((response) => {
        if (response.status === 200) {
          const dados = response.data.data;
          let dadosProcessados = dados.map((venda) => {
  ...
  const pesquisaData = (dataInicio, dataFim) => {
    axios.get(`https://back.pedrohenriq1921.repl.co/api/vendas?sort=data:desc&filters[data][$gte]=${dataInicio}&filters[data][$lte]=${dataFim}&populate=*`, config)
      .then((response) => {
        if (response.status === 200) {
          const dados = response.data.data;
          let dadosProcessados = dados.map((venda) => {
      ...
      .catch((error) => {
        ...
        //https://back.pedrohenriq1921.repl.co/api/item-vendas?filters[produto][id][$eq]=21&populate=*

  const pesquisaProduto = (produtoId) => {
    axios.get(`https://back.pedrohenriq1921.repl.co/api/item-vendas?filters[produto][id][$eq]=${produtoId}&sort=[venda][data]:desc&populate=*`, config)
      .then((response) => {
        if (response.status === 200) {
          const dados = response.data.data;
          let dadosProcessados = dados.map((venda) => {
            return {
              key: venda.attributes.venda.data.id,
              cliente: venda.attributes.venda.data.attributes.nome,
              data: venda.attributes.data,
              pagamento: venda.attributes.pagamento,
              entrega: venda.attributes.entrega.toFixed(2),
              desconto: venda.attributes.desconto.toFixed(2),            }
